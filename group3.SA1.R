...
param_table <- data.frame(
  Parameter   = c("Shape (k)", "Scale (lambda)"),
  Estimate    = c(shape_hat, scale_hat),
  SE          = c(best_model$res["shape", "se"],   best_model$res["scale", "se"]),
  Lower_95_CI = c(best_model$res["shape", "L95%"], best_model$res["scale", "L95%"]),
  Upper_95_CI = c(best_model$res["shape", "U95%"], best_model$res["scale", "U95%"])
)

print(param_table)
kable(param_table, digits = 3, caption = "MLE Parameter Estimates for Weibull Model")

#######################################
# Selected survival probabilities
#######################################
selected_times <- c(365, 730, 1095, 1825)

surv_probs <- data.frame(
  Time_days = selected_times,
  Survival_Probability = exp(-(selected_times / scale_hat)^shape_hat)
)

print(surv_probs)
kable(surv_probs, digits = 4, caption = "Survival Probabilities at Selected Time Points")
